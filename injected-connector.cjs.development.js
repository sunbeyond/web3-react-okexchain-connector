...
function (_Error) {
  ...
  function NoEthereumProviderError() {
    ...
    _this.message = 'No Ethereum provider was found on window.okexchain.';
    return _this;
  }
...
function (_AbstractConnector) {
  ...
  _proto.activate = function activate() {
    try {
      var _temp5 = function _temp5(_result) {
        ...
        var _temp = function () {
          if (!account) {
            ...
            return Promise.resolve(window.okexchain.enable().then(function (sendReturn) {
              return sendReturn && parseSendReturn(sendReturn)[0];
            })).then(function (_window$okexchain$enab) {
              account = _window$okexchain$enab;
            });
          }
        }();
      ...
      if (window.okexchain.on) {
        window.okexchain.on('chainChanged', _this5.handleChainChanged);
        window.okexchain.on('accountsChanged', _this5.handleAccountsChanged);
        window.okexchain.on('close', _this5.handleClose);
      ...
      var _temp6 = _catch(function () {
        return Promise.resolve(window.okexchain.send('eth_requestAccounts').then(function (sendReturn) {
          return parseSendReturn(sendReturn)[0];
        })).then(function (_window$okexchain$send) {
          account = _window$okexchain$send;
        });
      }, function (error) {
        if (error.code === 4001) {
  ...
  _proto.getProvider = function getProvider() {
    try {
      return Promise.resolve(window.okexchain);
    } catch (e) {
      return Promise.reject(e);
    }
  };
    ...
    try {
      var _temp12 = function _temp12() {
        function _temp9() {
          if (!chainId) {
            try {
              chainId = parseSendReturn(window.okexchain.send({
                method: 'net_version'
              }));
            } catch (_unused) {
          ...
          if (!chainId) {
            if (window.okexchain.isDapper) {
              chainId = parseSendReturn(window.okexchain.cachedResults.net_version);
            } else {
              chainId = window.okexchain.chainId || window.okexchain.netVersion || window.okexchain.networkVersion || window.okexchain._chainId;
            }
          }
        ...
        var _temp8 = function () {
          if (!chainId) {
            var _temp14 = _catch(function () {
              return Promise.resolve(window.okexchain.send('net_version').then(parseSendReturn)).then(function (_window$okexchain$send3) {
                chainId = _window$okexchain$send3;
              });
            }, function () {
      ...
      var _temp13 = _catch(function () {
        return Promise.resolve(window.okexchain.send('eth_chainId').then(parseSendReturn)).then(function (_window$okexchain$send2) {
          chainId = _window$okexchain$send2;
        });
      }, function () {
  ...
  _proto.getAccount = function getAccount() {
    try {
      var _temp20 = function _temp20() {
        function _temp17() {
          if (!account) {
            account = parseSendReturn(window.okexchain.send({
              method: 'eth_accounts'
            }))[0];
          }
        ...
        var _temp16 = function () {
          if (!account) {
            var _temp22 = _catch(function () {
              return Promise.resolve(window.okexchain.enable().then(function (sendReturn) {
                return parseSendReturn(sendReturn)[0];
              })).then(function (_window$okexchain$enab2) {
                account = _window$okexchain$enab2;
              });
            }, function () {
      ...
      var _temp21 = _catch(function () {
        return Promise.resolve(window.okexchain.send('eth_accounts').then(function (sendReturn) {
          return parseSendReturn(sendReturn)[0];
        })).then(function (_window$okexchain$send4) {
          account = _window$okexchain$send4;
        });
      }, function () {
  ...
  _proto.deactivate = function deactivate() {
    if (window.okexchain && window.okexchain.removeListener) {
      window.okexchain.removeListener('chainChanged', this.handleChainChanged);
      window.okexchain.removeListener('accountsChanged', this.handleAccountsChanged);
      window.okexchain.removeListener('close', this.handleClose);
      window.okexchain.removeListener('networkChanged', this.handleNetworkChanged);
    }
  };
